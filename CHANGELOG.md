...
### Интерфейс больше не подвисает на 7 секунд

- Поиск Node собирал список путей целиком до проверки, поэтому `where.exe` и
  `powershell.exe` запускались всегда — даже когда Node лежал по первому же пути.
  Пять синхронных процессов подряд блокировали сервер, и на это время он не отвечал
  ни на один запрос. Дорогие проверки теперь выполняются только если все дешёвые
  пути промахнулись, а перебор дисков заходит лишь на смонтированные буквы.
- Повторное сканирование в `/api/setup` запускается только пока что-то не найдено:
  уже определённый путь и так перепроверяется на каждом вызове. `/api/setup`
  отвечает за 7 мс вместо 7.6 секунды.

### Вкладка «Чаты»

- Список стал адаптивной сеткой карточек (до трёх колонок), скроллит сама страница —
  раньше внутренний скроллбокс давал второй скроллбар и обрезанную нижнюю строку.
- В карточке иконка, заголовок, время последнего изменения и путь проекта, обрезанный
  с начала: раньше клипался хвост и вместо имени папки был виден общий префикс.
- В шапке счётчик сохранённых чатов, карточка целиком кликабельна и работает
  с клавиатуры, список кэшируется на 30 секунд.

### Оптимизация и мелочи

- Конфиг, проверка OmniRoute и собранный `index.html` кэшируются; браузер при старте
  открывается асинхронно (раньше `execFileSync` мог задержать запуск до 31 секунды).
- `/api/logs` читает хвост файла вместо всего лога.
- Исправлен вызов удалённой функции в момент успешного входа: авторизация проходила,
  но модалка зависала в состоянии ожидания вместо «подключено».
- Иконки моделей: первая неудачная загрузка с CDN откладывает попытки на сутки, так что
  на заблокированной сети запросы больше не уходят при каждой перерисовке.
- Статус OmniRoute переведён — раньше строки были захардкожены по-английски.

## 1.3.7 — 2026-08-16

### Вкладка «Логи»

- Полный лог FreeClaude пишется в `%APPDATA%\\FreeClaude\\logs\\freeclaude.log` без
  урезания (установка, OmniRoute, ошибки UI, console.error).
- В сайдбаре вкладка **Логи**: просмотр всего файла, автообновление, скачать `.txt`,
  очистить.

## 1.3.6 — 2026-08-16

### OmniRoute снова поднимается после смены аккаунта

- В 1.3.4 OmniRoute убивали дважды (logout + после логина). Порт 20128 оставался
  занят сиротой: FreeClaude писал «не поднялся», а ручной `omniroute` ругался, что
  порт уже занят.
- Теперь один аккуратный рестарт при выходе: ждём, пока порт реально свободен,
  при «мёртвом» слушателе освобождаем и пробуем ещё раз. После логина второй
  kill/start больше не делается.

## 1.3.5 — 2026-08-16

### Запуск Claude: `я╗┐@echo` / «не является командой»

- `.bat` / `.cmd` писались в UTF-8 **с BOM**. `cmd.exe` читал BOM как мусор в первой
  строке и ругался на `я╗┐@echo`. Пути уже через `%APPDATA%`, поэтому BOM не нужен —
  файлы снова пишутся как обычный ASCII/UTF-8 без BOM.

## 1.3.4 — 2026-08-16

### Лимит больше не «липнет» на новый аккаунт Kiro

- После 402 OmniRoute держал исчерпание в памяти и в `quota_snapshots`. Смена Builder ID
  обновляла токены, но старый лимит оставался — новый аккаунт тоже отвечал «reached the limit».
- При выходе/смене аккаунта FreeClaude полностью удаляет Kiro-сессии и снимки квоты,
  затем перезапускает OmniRoute, чтобы сбросить кэш «all accounts exhausted».
- Баннер лимита смотрит только на живые аккаунты: здоровый сосед больше не блокируется
  из-за исчерпанного хвоста.

## 1.3.2 — 2026-08-13

### Режим Axiom работает у всех из коробки

- Промпт входит в сборку (`axiom-default.md` рядом с `FreeClaude.exe`).
- При первом запуске текст копируется в данные FreeClaude; включение тумблера
  ставит его в `~/.claude/CLAUDE.md`. Выключение убирает файл, но текст сохраняется.
- Карточка снова видна у новых пользователей — «Промпт в комплекте — включи, чтобы установить».

## 1.3.1 — 2026-08-12

### Запуск Claude Code больше не падает с «путь не найден»

- Вложенный `omniroute launch` и абсолютный путь к `.bat` ломались у пользователей
  с кириллицей в имени папки Windows: cmd.exe читал UTF-8 без BOM как кракозябры
  и отвечал «Системе не удается найти указанный путь».
- Claude Code теперь стартует напрямую, `.cmd` пишется с BOM, а вызов идёт через
  `%APPDATA%\FreeClaude\freeclaude.bat`.

### OmniRoute: 502 и пароль панели

- Список моделей и проверка ответа сами поднимают OmniRoute, если он ещё не online —
  вместо сразу «502 · Нет связи с OmniRoute».
- Пароль панели ищется и в `%APPDATA%\omniroute`, не только в `~/.omniroute`.
- Автосброс пароля можно повторить, если в первый раз OmniRoute ещё не был готов.
- Тост больше не пугает «часто CHANGEME», когда FreeClaude уже пытается сбросить пароль сам.

### Режим персоны

- Карточка скрыта, если текста промпта нет. В релизе он не поставляется — у новых
  пользователей мёртвый тумблер больше не висит в Настройках.
